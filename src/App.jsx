...
export default function App() {
  ...
  const handleSetSystemTheme = (val) => {
    setSystemTheme(val);
  ...
  return <div className="relative min-h-screen bg-slate-50 dark:bg-[#030303] text-slate-800 dark:text-slate-100 transition-colors overflow-x-hidden">
      {
    /* Mesh Gradient Background Layers for Frosted Glass Theme */
      ...
      <AnimatePresence mode="wait">
        {loading ? <LoadingScreen key="loading" onComplete={() => setLoading(false)} /> : <motion.div
    key="content"
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
