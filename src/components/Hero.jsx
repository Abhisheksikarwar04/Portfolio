...
import { ArrowRight, Download, Mail, Terminal, Code2, Cpu, Github, Linkedin, Briefcase } from "lucide-react";
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      particles = [];
      const particleCount = Math.min(60, Math.floor(canvas.width * canvas.height / 25e3));
    ...
    const animate = () => {
      ...
      particles.forEach((p, index) => {
        ...
        ctx.fillStyle = isDark ? `rgba(165, 180, 252, ${p.alpha})` : `rgba(79, 70, 229, ${p.alpha * 0.6})`;
        ctx.fill();
        for (let j = index + 1; j < particles.length; j++) {
          ...
          if (dist < 120) {
            ...
            ctx.strokeStyle = isDark ? `rgba(165, 180, 252, ${(1 - dist / 120) * 0.15})` : `rgba(79, 70, 229, ${(1 - dist / 120) * 0.1})`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setTypedText(
        isDeleting ? currentRole.substring(0, charIndex - 1) : currentRole.substring(0, charIndex + 1)
      );
