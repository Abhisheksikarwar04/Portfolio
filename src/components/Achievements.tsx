...
function Counter({ val, trigger }: { val: string; trigger: boolean }) {
  const [count, setCount] = useState(0);

  // Extract base numerical value
  ...
  useEffect(() => {
    ...
    const increment = Math.ceil(numValue / (duration / 16)); // ~60fps refresh rate
  ...
  return (
    <p className="text-3xl sm:text-4xl font-display font-black text-slate-900 dark:text-white leading-none">
      {trigger ? count : 0}
