...
export default function Projects({ addToast }) {
  ...
  const filteredProjects = useMemo(() => {
    return projectsData.filter((project) => {
      ...
      const matchesSearch = project.title.toLowerCase().includes(searchQuery.toLowerCase()) || project.description.toLowerCase().includes(searchQuery.toLowerCase()) || project.techStack.some(
  ...
  />
    ...
    className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all cursor-pointer ${selectedCategory === cat ? "bg-indigo-600 text-white shadow-md shadow-indigo-500/10" : "bg-white/10 hover:bg-white/20 dark:bg-white/5 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 border border-slate-200/20 dark:border-white/10"}`}
  ...
  >
    ...
    className={`group relative rounded-3xl overflow-hidden glass transition-all hover:shadow-2xl hover:scale-[1.01] duration-300 ${project.featured ? "border-indigo-500/30 dark:border-indigo-500/30 shadow-md shadow-indigo-500/[0.05]" : ""}`}
    id={`project-card-${project.id}`}
  >
                {
    /* Featured Ribbon badge */
  }
                {project.featured && <div className="absolute top-4 right-4 z-10 flex items-center gap-1 px-3 py-1 bg-indigo-600 text-white rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm">
  ...
  >
    ...
    /* Technology list tags */
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {project.techStack.slice(0, 4).map((tech) => <span
    key={tech}
  ...
  >
                      ...
                      {project.techStack.length > 4 && <span className="px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
  ...
  >
    ...
    /* Empty Search Fallback */
        ...
        {filteredProjects.length === 0 && <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
  ...
  >
    ...
    /* Embedded Project Details Modal */
      ...
      <AnimatePresence>
        {activeProjectModal && <ProjectModal
