...
export default function Navbar({ darkMode, toggleDarkMode, systemTheme, setSystemTheme }: NavbarProps) {
  ...
  // Track scroll position for navbar background and scroll progress
  useEffect(() => {
    const handleScroll = () => {
  ...
  useEffect(() => {
    const observerOptions = {
      root: null,
      rootMargin: "-20% 0px -60% 0px", // Trigger when the section occupies a significant middle part of screen
      threshold: 0
    };
              ...
              <div className="border-t border-slate-200/50 dark:border-slate-800/60 mt-3 pt-3 flex items-center justify-between">
                ...
                <div className="flex bg-slate-100 dark:bg-slate-800/60 p-1 rounded-lg border border-slate-200/50 dark:border-slate-700/50">
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-md transition-all font-medium flex items-center gap-1.5 ${
                      !systemTheme && !darkMode
                        ? "bg-white dark:bg-slate-900 text-amber-500 shadow-sm"
                        : "text-slate-400"
                    }`}
